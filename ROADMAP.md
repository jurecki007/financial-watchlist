...
- [x] Alert mail fixed and the alerts page stopped overclaiming — the sender fell back to `finance-demo…` while the site is `financial-demo…`, so once Resend's verified domain changed, three consecutive alerts were rejected with a 403 that creates no email record. Nothing surfaced it, because the page labelled `triggered_at` as "emailed" and asserted mail that never left. Sender corrected, label now says `triggered`, which is what the column actually knows
- [x] Every email is readable on `/about/project` — all four framed from the same files that get sent, so a reviewer sees them without triggering anything and the page cannot drift from the inbox. Needed `X-Frame-Options` narrowed to `SAMEORIGIN` for `/mockups/` alone; the blanket `DENY` blocked the frames silently, which only rendering the page revealed
- [x] Price alerts are actually evaluated — the schedule existed only as a sentence in the README, so alerts sat pending for days and the single alert that ever fired was fired by hand. Now an hourly `pg_cron` job committed as a migration (one Twelve Data credit per sweep, 24/day), plus an immediate scoped check when an alert is created so an already-met threshold does not look broken for an hour
- [x] Alert claiming made a compare-and-set — running hourly *and* on creation lets two evaluations meet on one row, and the previous blind write would have let both send. Proven by racing three concurrent claims and confirming one winner
