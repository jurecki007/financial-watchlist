...
- **The UI states what a column means, not what we hope it implies.** `triggered_at` records that the evaluator *claimed* an alert, which happens before the send precisely so a crash cannot deliver one crossing twice. The alerts page labelled it "emailed", so when the Resend sender domain stopped being verified and three consecutive sends were rejected, the product went on reporting mail it had never sent. A rejected Resend request creates no email record either, so the dashboard showed nothing — the false label was the only thing anyone could see. If delivery is to be claimed, it needs its own column set after the provider accepts.
- **The mail sender must sit on the domain the site is served from.** `ALERTS_FROM` fell back to `alerts@finance-demo…` while the site is `financial-demo…`; the typo was invisible while Resend happened to have the wrong domain verified, and became a total send outage the moment that changed. A mismatched sender is also what a phishing attempt looks like, which is expensive for a demo whose job is to be believed.
