...
- **Seeded price alerts must not be able to fire.** `check-price-alerts` emails the account's own address, and the demo account's address is on a domain we do not own, so a triggered seed would put automated mail on a third party's server and earn a bounce against the Resend sending domain. Seed the pending state with an unreachable threshold and the fired state with `triggered_at` already set and `active = false` — the evaluator scans `where active and triggered_at is null`, so that row is outside its query by construction rather than by luck.
